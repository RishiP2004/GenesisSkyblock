...
class AddUluruCommand extends BaseCommand {
	use PlayerCallTrait;

	public function prepare() : void {
		$this->setPermission("adduluru.command");
	...
	public function onRun(CommandSender $sender, string $commandLabel, array $args) : void {
		$this->getCoreUser($args["player"], function($user) use ($sender, $args) {
			if(is_null($user)) {
				...
				$user->addUluru((int) $args["amount"]);

				$player = Server::getInstance()->getPlayerByPrefix($user->getName());
		
				if($player instanceof CorePlayer) {
					$player->sendMessage(Skyblock::PREFIX . $sender->getName() . " gave you " . $args["amount"] . " uluru");
				}
				$sender->sendMessage(Skyblock::PREFIX . "Gave " . $user->getName() . " " . $args["amount"] . " uluru");
				return true;
			}
		});
    }
