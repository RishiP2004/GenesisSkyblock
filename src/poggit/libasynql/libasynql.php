...
final class libasynql{
	 ...
	 * @param string[]|string[][] $sqlMap     an associative array with key as the SQL dialect ("db", "sqlite") and value as a string or string array indicating the relevant SQL files in the plugin's resources directory
	...
	public static function create(PluginBase $plugin, $configData, array $sqlMap, bool $logQueries = null) : DataConnector{
		...
		switch(strtolower($type)){
			...
			case "mysqli":
				...
				$cred = MysqlCredentials::fromArray($configData["db"], strtolower($plugin->getName()));
